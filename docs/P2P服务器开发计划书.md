...
>
> 编号约定：新工作用 **档 A / B / C / D**。`P0–P11` 只在附录对照历史阶段，不再当待办清单。
> 「连接核 vs 流媒体平台还缺什么」见 [`P2P流媒体服务器能力缺口.md`](P2P流媒体服务器能力缺口.md)。
...
|--------|----|
| 现在做到哪、下一档是什么 | §2 |
| 连接核之外还缺什么 | [`P2P流媒体服务器能力缺口.md`](P2P流媒体服务器能力缺口.md) |
| 进程怎么拆、流量怎么走 | §3 |
| 和 TUTK 差在哪 | §4 |
| 配置键 / 目录不能再堆什么 | §5 |
| 协议与选路（已落地） | §6 |
...
| [`P2P流媒体服务器能力缺口.md`](P2P流媒体服务器能力缺口.md) | 做成流媒体**平台**还缺哪一层 |
...
一句话：**连接核已收口。** 可售的「P2P 流媒体服务器」还缺平台面（档 D）和现网（档 B）；
1:N 才需要旁路网关（档 C）。不是再拆 NatServer。

| 档 | 内容 | 状态 | 缺的是 |
|----|------|------|--------|
| **A. 连接核** | 信令、中继、UID/Token、IOTC/AV/RDT/Tunnel、ICE/DERP、同步/状态口 | **已收口** | 小项见 §7.4 |
| **B. 现网验收** | 10 万心跳、弱网 1080p、家宽开孔、443 证书、三平台 SDK、唤醒 <6s、NAT 矩阵 | **待环境** | 机器、证书、真设备 |
| **C. 旁路网关** | 多看客 SFU、28181、WHIP/WHEP、MoQ | **未立项** | 独立进程，禁止进信令核 |
| **D. 平台面** | Master 授权、中继按 UID 计量、会话遥测、客户端锁区、PLI 约定 | **未建** | 独立服务 + 少量计数器；细节见缺口文档 |
...
设备低频 `MSG_WAKE_KEEPALIVE`（可选 `WakeSecret`）。CONNECT 目标离线时 NatServer 通知 wakeserver `TRIGGER`，向上次公网地址 `POKE`。Trigger 本身尚无 HMAC（协议扩字段需兼容期，见 §7.4）。
...
### 7.1 档 D — 平台面（与现网并行，D3 优先）

连接核不是完整「流媒体平台」。TUTK 另有 Master / KDC / 锁区；WebRTC 生产还要
会话级观测和计费。**不要焊进 `p2p_natserver` 热路径。** 完整表见缺口文档 §3.4。

| 优先级 | 项 | 验收 |
|--------|----|------|
| D3 | 每条 CONNECT 记 `path_kind` + NAT 分母；客户端上报 TTFF | 能报锥×锥直连率、中继占比 |
| D2 | Proxy 按 UID 导出中继字节（QuotaMB 只是限速） | 对标 KDC 台账；直连不计费 |
| D6 | IOCtrl 请求 I 帧（PLI） | 花屏可恢复；服务端不解码 |
| D1 | Master：授权 P2P 节点、入口解析 | 多节点私有化可卖 |
| D4 | 客户端/设备锁区，与节点 Region 不一致则拒绝 | GDPR / 跨境 |
| D5 | 租户/VPG（License 白名单可当 v1） | 多客户一套集群 |

私有化只做 1:1 预览：D3 + 档 B 即可，D1/D4 可缓。
...
| B3 | 分 NAT 类型埋点（锥与对称分母分开） | 直连率口径可报 | 依赖 D3 会话遥测；现网或 docker+iptables |
...
```
档 A 连接核 ──已收口──► D3 会话遥测 ──► 档 B 现网灰度 ──► GA
                         │                    │
                         ├─► D2 计量 / D1 Master
                         └─► 档 C 仅 1:N / 浏览器（C1 → C2，C3 只调研）
```
...
7. 会话遥测（path / NAT 分母 / TTFF）、中继按 UID 台账、可选 Master/锁区

**待旁路（档 C）**

8. SFU / 28181 / WHIP 网关进程与 MoQ 笔记 —— **不进 NatServer**

---

## 附录：历史阶段编号对照

只为读旧文档 / 旧提交。新计划用档 A/B/C/D。
